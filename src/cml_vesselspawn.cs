...
namespace cml_vesselspawn
{
    public class VesselIntro : UpdatableAndDeletable
    {
        public VesselIntro(Room room)
        {
            this.room = room;
        }
        public override void Update(bool eu)
        {
            ...
            if (room.game.AllPlayersRealized)
            {
                ...
                for (int i = 0; i < this.room.game.Players.Count; i++)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        (this.room.game.Players[i].realizedCreature as Player).bodyChunks[j].HardSetPosition(this.room.MiddleOfTile(47, 30));
                    }
